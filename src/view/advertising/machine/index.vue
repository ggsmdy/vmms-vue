<template>
  <div>
    <Card>
      <!--<Dropdown trigger="click" @on-click="setReadStatus" style="margin: 0 5px 10px 10px">
        <Button type="primary">
          设置为
          <Icon type="ios-arrow-down"></Icon>
        </Button>
        <DropdownMenu slot="list">
          <DropdownItem name="read">已读</DropdownItem>
          <DropdownItem name="unread">未读</DropdownItem>
        </DropdownMenu>
      </Dropdown>
      <Button type="info" v-on:click="dispatchHander">派单</Button>-->
      <span>机器名称</span>
      <Input clearable placeholder="输入机器名称搜索" class="search-input"/><!--v-model="searchValue"-->
      <span>机器编号</span>
      <Input clearable placeholder="输入机器名称搜索" class="search-input"/>
      ...
      <!--<Dropdown trigger="click" @on-click="setReadStatus" style="margin: 0 5px 10px 10px">
        ...
        <DropdownMenu slot="list">
          <DropdownItem name="read">警告</DropdownItem>
          <DropdownItem name="unread">严重</DropdownItem>
          <DropdownItem name="unread1">崩溃</DropdownItem>
          <DropdownItem name="unread2">错误</DropdownItem>
        </DropdownMenu>
      </Dropdown>-->
  ...
  export default {
    ...
    data () {
      return {
        columns: [
          {
            title: '广告编号', key: 'id', align: 'center', sortable: false
          },
          {
            title: '已读',
            key: 'is_read',
            ...
            render: (h, params) => {
              return h('Icon',
                {props: {
                    size: '20',
                    color: params.row.is_read ? 'gray' : 'red',
                    type: params.row.is_read ? 'md-mail-open' : 'md-mail'
                  }})
            }
          },
          {
            title: '机器名称', key: 'machine_name', align: 'center', sortable: false
          },
          {
            title: '等级',
            key: 'level',
